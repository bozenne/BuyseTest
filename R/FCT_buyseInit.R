...
initCensoring <- function(censoring,data,endpoint,type,D,D.TTE,
                          treatment,strata,callMethod="initCensoring"){
  
  if(is.null(censoring)){
    
    if(D.TTE>0){
      stop(callMethod," : \'censoring\' must be specifyied (non NULL) when there are TTE endpoints \n",
           "number of Time To Event endpoints : ",D.TTE,"\n",
           "Time To Event endpoints : ",paste(endpoint[type==3],collapse=" "),"\n")
    }
    
  }else{
    if(length(censoring) != D){
      stop(callMethod," : \'censoring\' does not match \'endpoint\' size \n",
           "length(censoring) : ",length(censoring),"\n",
           "length(endpoint) : ",D,"\n")
    }
    
    if(any(!is.na(censoring[type!=3])) ){
      stop(callMethod," : wrong specification of \'censoring\' \n",
    ...
    if(any(censoring %in% names(data) == FALSE)){
      stop(callMethod," : wrong specification of \'censoring\' \n",
           "some columns requested are missing in data \n",
           "missing censoring : ",paste(censoring[censoring %in% names(data) == FALSE],collapse=" "),"\n",
           "available censoring : ",paste(names(data)[names(data) %in% c(endpoint,treatment,censoring,strata) == FALSE],collapse=" "),"\n")
    }
...
initCPU <- function(cpus,callMethod="initCPU"){
    
  if(cpus=="all"){ 
    cpus <- parallel::detectCores() # this function detect the number of CPU cores 
  }
  
  if(cpus!=1){ # if several cpus are intended to be used, check this correspond to a valid number of CPU cores
    if(!is.numeric(cpus) || cpus <= 0 || (cpus %% 1 !=0) || cpus>parallel::detectCores()){
      stop(callMethod," : wrong specification of \'cpus\' \n",
           "available number of cpus : ",parallel::detectCores()," \n",
           "proposed number of cpus : ",cpus,"\n")
    }
  }
...
initData <- function(data,Ind.Treatment,type,endpoint,D,censoring,
                     index.strataT,index.strataC,n.strata,          
                     method,D.TTE,threshold,Wscheme=NULL,
                     trace,test=TRUE,callMethod="initData"){
  
  ## creation
  data <- data[,c(endpoint,censoring),drop=FALSE] # restrict the dataset to the endpoint and censoring variable
  # strata and treatment variable have already been extracted.
  
  if(test && any(type==3)){
    test.na <- colSums(is.na(data[,c(censoring,endpoint[type==3]),drop=FALSE]))
    if(any(test.na>0)){ 
      stop(callMethod," : TTE endpoint / censoring variables must not contain NA \n",
  ...
  if(test && trace>0 && any(type!=3)){
    test.na <- colSums(is.na(data[,endpoint[type!=3],drop=FALSE]))
    if(any(test.na>0)){ 
      warning(callMethod," : some binary or continuous endpoints contains NA \n",
              "number of NA in \'data\' : ",sum(test.na>0),"\n",
              "columns with \'data\' : ",paste(c(endpoint,censoring[type==3])[test.na>0],collapse=" "),"\n")
    }
  }
  ...
  ## transformation
  M.Treatment <- as.matrix(data[Ind.Treatment==1,endpoint,drop=FALSE]) # matrix of endpoints for the treatment arm 
  M.Control <- as.matrix(data[Ind.Treatment==0,endpoint,drop=FALSE]) # matrix of endpoints for the control arm
  
  if(!is.null(censoring)){
    M.delta_Treatment <- as.matrix(data[Ind.Treatment==1,censoring,drop=FALSE]) # matrix of censoring variables for the treatment arm : censored (0) event time (1)
    M.delta_Control <- as.matrix(data[Ind.Treatment==0,censoring,drop=FALSE]) # matrix of censoring variables for the treatment arm : censored (0) event time (1)
  ...
  index_tempo <- which(type==1)
  if(test && length(index_tempo)>0){
    
    index_tempo2 <- colSums(as.matrix(M.Treatment[,index_tempo,drop=FALSE] %in% c(NA,0,1) == FALSE))
    if(any(index_tempo2>0)){
      stop(callMethod," : \'treatment\' does not match \'type\' \n",
           "binary endpoints : ",paste(endpoint[index_tempo],collapse=" ")," must contains only 0 or 1 (or NA) \n",
           "incorrect endpoints for the Treatment arm : ",paste(endpoint[index_tempo[index_tempo2>0]],collapse=" "),"\n")
    }
    
    index_tempo2 <- colSums(as.matrix(M.Control[,index_tempo,drop=FALSE] %in% c(NA,0,1) == FALSE))
    if(any(index_tempo2>0)){
      stop(callMethod," : \'Control\' does not match \'type\' \n",
           "binary endpoints : ",paste(endpoint[index_tempo],collapse=" ")," must contains only 0 or 1 (or NA) \n",
           "incorrect endpoints for the Control arm : ",paste(endpoint[index_tempo[index_tempo2>0]],collapse=" "),"\n")
    }
  }
  
  ## endpoint checking : time to event
  index_tempo <- which(type==3)
  if(test && length(index_tempo)>0){
    
    index_tempo2 <- colSums(as.matrix(M.delta_Treatment %in% c(0,1) == FALSE))
    if(any(index_tempo2>0)){
      stop(callMethod," : incorrect \'censoring\' variables (Treatment) \n",
           "Time to event censoring variables : ",paste(censoring[index_tempo],collapse=" ")," must contains only 0 or 1 \n",
           "incorrect \'censoring\' (Treatment) : ",paste(censoring[index_tempo[index_tempo2>0]],collapse=" "),"\n")
    }
    
    index_tempo2 <- colSums(as.matrix(M.delta_Control %in% c(0,1) == FALSE))    
    if(any(index_tempo2>0)){
      stop(callMethod," : incorrect \'censoring\' variables (Control) \n",
           "Time to event censoring variables : ",paste(censoring[index_tempo],collapse=" ")," must contains only 0 or 1 \n",
           "incorrect \'censoring\' (Control) : ",paste(censoring[index_tempo[index_tempo2>0]],collapse=" "),"\n")
    }
    
    index_tempo2 <- which(colSums( M.Treatment[,index_tempo,drop=FALSE] < 0) > 0)
    if(length(index_tempo2)>0){
      stop(callMethod," : incorrect \'Treatment\' variable \n",
           "Time to event endpoints : ",paste(endpoint[index_tempo],collapse=" ")," must contains only 0 or positive numbers  \n",
           "incorrect \'Treatment\' endpoints : ",paste(endpoint[index_tempo[index_tempo2]],collapse=" "),"\n")
    }
    
    index_tempo2 <- which(colSums( M.Control[,index_tempo,drop=FALSE] < 0) > 0)
    if(length(index_tempo2)>0){
      stop(callMethod," : incorrect \'Control\' variable \n",
           "Time to event endpoints : ",paste(endpoint[index_tempo],collapse=" ")," must contains only 0 or positive numbers  \n",
           "incorrect \'Control\' endpoints : ",paste(endpoint[index_tempo[index_tempo2]],collapse=" "),"\n")
    }
    
  }
  
  ####  KM imputation  #### 
  if(test && method %in% c("Gehan","Peto","Efron","Peron") == FALSE){
    stop(method," : incorrect \'method\' variable \n",
         "valid \'method\' values : \"Gehan\" \"Peto\" \"Efron\" \"Peron\" \n",
  ...
  if(method %in% c("Peto","Efron","Peron")){
    ...
    res_init <- initSurvival(M.Treatment=M.Treatment,M.Control=M.Control,M.delta_Treatment=M.delta_Treatment,M.delta_Control=M.delta_Control,
                             ...
                             method=method,callMethod=callMethod)
...
initEndpoint <- function(endpoint,data,D,
                         treatment,censoring,strata,callMethod="initEndpoint"){
  
  if(length(endpoint)!=D){
    stop(callMethod," : \'endpoint\' length must match \'type\' length \n",
         "length(endpoint) : ",length(endpoint)," \n",
         "length(type) : ",D," \n")
  }
  
  if(any(endpoint %in% names(data) == FALSE)){
    stop(callMethod," : wrong specification of \'endpoint\' \n",
         "some columns requested are missing in data \n",
         "missing endpoints : ",paste(endpoint[endpoint %in% names(data) == FALSE],collapse=" "),"\n",
         "available variables in data : ",paste(names(data)[names(data) %in% c(endpoint,treatment,censoring,strata) == FALSE],collapse=" "),"\n")
  }
  
}

initN <- function(n,callMethod="initN"){
  
  #### tests ####
  
  if(length(n)!=3){
    stop(callMethod," : wrong specification of \'n\' \n",
...
initHypothesis <- function(hypothesis,type,D,callMethod="initParamSimul"){
  ...
  if(!is.list(hypothesis) || length(hypothesis)!=D){
    stop(callMethod," : proposed \'hypothesis\' does not match \'type\' \n",
  ...
  for(iter_outcome in 1:D){
    ...
    if(any(parameters_tempo %in% names(hypothesis[[iter_outcome]]) == FALSE)){
      stop(callMethod," : wrong specification of \'hypothesis\' \n",
           "type of outcome ",iter_outcome," : ",names(valid_parameters)[type[iter_outcome]]," \n",
    ...
    for(iter_param in 1:length(parameters_tempo)){
      if(parameters_tempo[iter_param] %in% names(valid_range)){
        ...
        if(test.inf || test.sup){
          stop(callMethod," : unvalid parameter value in \'hypothesis\' \n",
...
initAlloc <- function(proba,n.Treatment,n.Control,callMethod="initAlloc"){
  
  if(is.null(proba)){ # if prob.alloc is not set by the user
    proba <- n.Treatment/(n.Treatment+n.Control)  # it is set to the proportion of patients in the treatment arm.
  }
  
  if(proba<=0 || proba>=1){
    stop(callMethod," : wrong specification of \'prob.alloc\' \n",
         "\'prob.alloc\' is a probability and need to be between 0 and 1 \n",
         "proposed \'prob.alloc\' : ",proba,"\n")
  }
...
initStrata <- function(strata,data,Ind.Treatment,n.Treatment,n.Control,
                       endpoint,treatment,censoring,callMethod="initStrata"){
  
  if(!is.null(strata)){
    if(strata %in% names(data) == FALSE){
      stop(callMethod," : \'strata\' is missing in data\n",
           "proposed \'strata\' : ",strata,"\n",
           "available variables in data : ",paste(names(data)[names(data) %in% c(endpoint,treatment,censoring,strata) == FALSE],collapse=" "),"\n")
    }
    
    strataT <- as.factor(as.character(data[Ind.Treatment==1,strata])) # strata variable for the treatment arm transformed into factor
    strataC <- as.factor(as.character(data[Ind.Treatment==0,strata])) # strata variable for the control arm transformed into factor
...
initThreshold <- function(threshold,type,D,method,endpoint,callMethod="initThreshold"){
  ...
  if(any(na.omit(threshold)==0)){
    threshold[na.omit(which(threshold==0))] <- 10^{-12}
  }
  
  if(length(threshold)!=D){
    stop(callMethod," : \'threshold\' length must match \'type\' length \n",
         "length(threshold) : ",length(threshold)," \n",
         "length(type) : ",D," \n")
  }
  
  if(any(threshold[type!=1]<0) || any(is.na(threshold[type!=1])) ){
    stop("BuyseTest : wrong specification of \'threshold\' \n",
         "\'threshold\' must only contains 0 or positive numbers for continuous or time to event endpoints \n",
         "proposed \'threshold\' : ",paste(threshold[type!=1],collapse=" "),"\n",
         "proposed endpoints : ",paste(endpoint[type!=1]),"\n")
  }
  ...
  if(method %in% c("Peto","Efron","Peron")){
    ...
    if(any(test.decreasing>0)){
      stop(callMethod," : wrong specification of \'endpoint\' or \'threshold\' \n",
...
initTreatment <- function(data,treatment,endpoint,censoring,strata,callMethod="initTreatment"){
  
  treatment_sauve <- treatment
  
  ## treatment
  if(length(treatment)!=1){
    stop(callMethod," : \'treatment\' must have length 1 \n",
         "length(treatment) : ",length(treatment)," \n")
  }
  
  if(treatment %in% names(data) == FALSE){
    stop(callMethod," : \'treatment\' is missing in data \n",
         "proposed \'treatment\' : ",treatment,"\n",
         "available variables in data : ",paste(names(data)[names(data) %in% c(endpoint,treatment,censoring,strata) == FALSE],collapse=" "),"\n")
  }
  
  Ind.Treatment <- as.factor(data[,treatment]) # treatment variable transformed into factor 
  reference <- levels(Ind.Treatment) # extraction of the levels of the treatment variable
  
  if(length(unique(Ind.Treatment)) != 2){
    stop(callMethod," : wrong specification of \'treatment\' \n",
         "the corresponding column in \'data\' must have exactly 2 levels \n",
         "proposed levels : ",paste(unique(Ind.Treatment),collapse=" "),"\n")
  }
  
  Ind.Treatment <- as.numeric(Ind.Treatment)-1 # treatment variable transformed into a binary variable : 1 (treatment) and 0 (control)
  n.Treatment <- sum(Ind.Treatment==1) # number of patient in the treatment arm
  n.Control <- sum(Ind.Treatment==0) # number of patient in the control arm
  
  #### export ####
  res <- list()
  res$Ind.Treatment <- Ind.Treatment
  res$reference <- reference
  res$n.Treatment <- n.Treatment
  res$n.Control <- n.Control
  
  return(res)
}

initType <- function(type,callMethod="initType"){
  
  ## type
  if(any(type %in% c("bin","binary","cont","continuous","TTE","timeToEvent")==FALSE)&&any(type %in% 1:3 == FALSE)){
    stop(callMethod," : wrong specification of \'type\' \n",
         "valid values : \"bin\" \"binary\" \"cont\" \"continuous\" \"TTE\" \"timeToEvent\" \n",
         "proposed value : ",paste(type,collapse=" "),"\n")
  }
