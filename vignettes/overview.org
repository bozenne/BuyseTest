...
#+Author: Brice Ozenne
#+LaTeX_HEADER: %\VignetteIndexEntry{Overview-BuyseTest}
#+LaTeX_HEADER: %\VignetteEngine{knitr::knitr}
...
* Perforing generalized pairwise comparisons (GPC) using the =BuyseTest= function
...
- the threshold associated to each endpoint (default\(=10^{-12}\))  \hfill - argument =threhsold=
