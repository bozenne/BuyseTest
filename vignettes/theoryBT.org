...
Let's take to random realisation of \(X\) and denote thoses \(X_1\)
...
So under the asymption that the first term of the H-decomposition of
...
#+BEGIN_EXPORT latex
\begin{align*}
H^{(1)}_{\hat{\sigma}^2} &= \frac{2}{n} \sum_{i=1}^n  \Esp[\frac{(x_i-X_2)^2}{2} \big|x]  - \sigma^2 = \frac{1}{n} \sum_{i=1}^n \Esp[x_i^2 - 2 x_i X_2 + X_2^2 \big|x_i]  - \sigma^2 \\
...
#+BEGIN_EXPORT latex
\begin{align*}
H_{\hat{U}}^{(2)} &= \frac{1}{mn} \sum_{i=1}^m \sum_{j=1}^n \Esp\left[ \Ind[x_i \leq y_j] \big| x_i,y_j\right] - \Esp\left[ \Ind[x_i \leq y] \big| x_i\right] - \Esp\left[ \Ind[x \leq y_j] \big| y_j\right] \\
&= \frac{1}{mn} \sum_{i=1}^m \sum_{j=1}^n \Ind[x_i \leq y_j] - \Esp\left[ \Ind[x_i \leq y] \big| x_i\right] - \Esp\left[ \Ind[x \leq y_j] \big| y_j\right]
\end{align*}
#+END_EXPORT
Indeed:
#+BEGIN_EXPORT latex
...
\end{align*}
#+END_EXPORT
which under the null hypothesis that \(X\) and \(Y\) have the same distribution equals \(-\Cov\left[\Ind[x_i \leq y_j],\Esp\left[ \Ind[x \leq y_j]\right] \big| y_j\right]\).
  \clearpage 
