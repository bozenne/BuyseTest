...
* Parameter of interest :noexport:
...
* Inverse probability weighting  :noexport:
...
* Asymptotic distribution                        :noexport:
...
#+BEGIN_EXPORT latex
\begin{align*}
H^{(1)} = \frac{k_x}{m} \sum_{i=1}^{m} \left( \Esp[h(x,x_2,\ldots,x_{k_x},y_1,\ldots,y_{k_y}) \big| x] - U \right) + \frac{k_y}{n} \sum_{j=1}^{n} \left( \Esp[h(x_1,\ldots,x_{k_x},y,y_2,\ldots,y_{k_y}) \big| y] - U \right)
\end{align*}
#+END_EXPORT
...
# set.seed(10)
# d <- simBuyseTest(5)
# e.BT <- BuyseTest(Treatment ~ cont(score), data = d, method.inference = "asymptotic", keep.pairScore = TRUE)

# e.dtBT <- e.BT@tablePairScore[[1]][,.(index.C,index.T,favorable,unfavorable)]
# iid.BT <- cbind(favorable = c((e.dtBT[,mean(favorable),by = "index.C"][[2]] - mean(e.dtBT$favorable))/length(unique(e.dtBT$index.C)),
#                               (e.dtBT[,mean(unfavorable),by = "index.T"][[2]] - mean(e.dtBT$unfavorable))/length(unique(e.dtBT$index.T))),
#                 unfavorable = c((e.dtBT[,mean(unfavorable),by = "index.C"][[2]] - mean(e.dtBT$unfavorable))/length(unique(e.dtBT$index.C)),
#                                 (e.dtBT[,mean(favorable),by = "index.T"][[2]] - mean(e.dtBT$favorable))/length(unique(e.dtBT$index.T)))
#                 )

# colSums(iid.BT^2)
# e.BT@covariance

# colSums(iid.BT[d$Treatment=="T",]^2) + colSums(iid.BT[d$Treatment=="C",]^2)

# mean(e.dtBT[, sum(apply(combn(.N, m = 2),2, function(x){.SD[x[1],favorable]*.SD[x[2],favorable]}))/NCOL(combn(.N, m = 2)), by = "index.T"][[2]]) - mean(e.dtBT[, favorable])^2

# mean(e.dtBT[, sum(apply(combn(.N, m = 2),2, function(x){.SD[x[1],unfavorable]*.SD[x[2],unfavorable]}))/NCOL(combn(.N, m = 2)), by = "index.T"][[2]]) - mean(e.dtBT[, unfavorable])^2


  \clearpage 

* Information about the R session used for this document :noexport:
