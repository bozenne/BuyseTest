...
We can compute an *estimate* \(s\) of the variance using the following
*estimators* \(f_m\) and \(f_s\):
#+BEGIN_EXPORT latex
\begin{align}
...
quantity of interest. This is often refer to as a *functionnal*,
...
** Why using the U-statistic theory?
...
*Theorem 3* (cite:lee1990u) Let \(U_n\) be a U-statistic with a kernel \(h\) of degre \(r\). Then:
#+BEGIN_EXPORT latex
\begin{align*}
