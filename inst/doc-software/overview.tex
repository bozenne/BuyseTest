...
\tau]\) leads to the net treatment benefit\footnote{in absence of ties and
for an infinitesimal \(\tau\) the testing whether the net treatment
benefit is 0 is equivalent the Wilcoxon-Mann-Whitney parameter} and
their ratio \(\frac{\Prob[Y \geq X + \tau]}{\Prob[X \geq Y + \tau]}\)
to the win ratio. The software also evaluate the proportion of neutral
pairs \(\Prob[ |X - Y| < \tau]\) and which can be included to obtain
the probabilistic index \(\Prob[Y \geq X + \tau] + 0.5 \Prob[ |X - Y|
...
\item \texttt{confint} to extract estimates, confidence intervals, and
p.values.
\item \texttt{plot} for a graphical display of the scoring of the pair per
endpoint.
\item \texttt{sensitivity} to perform a sensitivity analysis on the choice of
the threshold(s).
...
\item \texttt{getPairScore} to extract the contribution of each pair in a data.frame format (long format).
\item \texttt{getMatrixScore} to extractthe contribution of each pair in a matrix format (squared format).
...
Before going further we need to load the \textbf{BuyseTest} package in the \Rlogo
session:
...
To illustrate the functionalities of the package, we use the \texttt{veteran}
dataset from the \textbf{survival} package:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
For completness, the details of the \Rlogo session used to generate this
document are:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
sessionInfo()
...
[1] prodlim_2026.03.11  ggplot2_4.0.2       survival_3.5-8      BuyseTest_3.3.9    
[5] butils.base_1.3     Rcpp_1.1.1-1        devtools_2.5.1      usethis_3.2.1      
[9] data.table_1.18.2.1

loaded via a namespace (and not attached):
 [1] tidyselect_1.2.1          dplyr_1.2.1               farver_2.1.2             
 [4] S7_0.2.1-1                fastmap_1.2.0             TH.data_1.1-5            
 [7] digest_0.6.39             rpart_4.1.23              lifecycle_1.0.5          
[10] cluster_2.1.6             ellipsis_0.3.3            magrittr_2.0.5           
[13] compiler_4.5.3            rlang_1.2.0               Hmisc_5.2-5              
[16] doSNOW_1.0.20             tools_4.5.3               knitr_1.51               
[19] htmlwidgets_1.6.4         timereg_2.0.7             pkgbuild_1.4.8           
[22] xml2_1.5.2                RColorBrewer_1.1-3        multcomp_1.4-30          
[25] pkgload_1.5.1             polspline_1.1.25          withr_3.0.2              
[28] foreign_0.8-86            purrr_1.2.2               numDeriv_2016.8-1.1      
[31] desc_1.4.3                nnet_7.3-19               grid_4.5.3               
[34] stats4_4.5.3              roxygen2_7.3.3            mets_1.3.9               
[37] colorspace_2.1-2          future_1.70.0             riskRegression_2026.05.19
[40] MASS_7.3-60.0.1           globals_0.19.1            scales_1.4.0             
[43] iterators_1.0.14          cli_3.6.6                 mvtnorm_1.3-7            
[46] rmarkdown_2.31            rms_8.1-1                 generics_0.1.4           
[49] otel_0.2.0                rstudioapi_0.18.0         future.apply_1.20.2      
[52] RcppArmadillo_15.2.4-1    sessioninfo_1.2.3         cachem_1.1.0             
[55] stringr_1.6.0             splines_4.5.3             parallel_4.5.3           
[58] base64enc_0.1-6           vctrs_0.7.3               sandwich_3.1-1           
[61] glmnet_4.1-10             Matrix_1.6-5              SparseM_1.84-2           
[64] Formula_1.2-5             htmlTable_2.4.3           listenv_0.10.1           
[67] foreach_1.5.2             cmprsk_2.2-12             snow_0.4-4               
[70] glue_1.8.1                parallelly_1.47.0         codetools_0.2-19         
[73] stringi_1.8.7             shape_1.4.6.1             gtable_0.3.6             
[76] tibble_3.3.1              pillar_1.11.1             quantreg_6.1             
[79] htmltools_0.5.9           lava_1.9.0                R6_2.6.1                 
[82] rprojroot_2.1.1           evaluate_1.0.5            lattice_0.22-5           
[85] backports_1.5.1           memoise_2.0.1             MatrixModels_0.5-4       
[88] nlme_3.1-163              gridExtra_2.3             checkmate_2.3.4          
[91] xfun_0.57                 zoo_1.8-15                fs_2.1.0                 
[94] pkgconfig_2.0.3
\end{verbatim}
...
The \texttt{model.tables} method outputs the information from the previous
table in a \texttt{data.frame} format:
...
which, in this example, is around 0.42.

\bigskip

\Warning Testing an inadequate null hypothesis can have dramatic
...
\begin{itemize}
\item Wilcoxon-Mann-Whitney parameter or probabilistic index: \(\Prob[Y\geq X + \tau] + 0.5 \; \Prob\left[|Y- X| < \tau\right]\).
\item win odds: \(\frac{\Prob[Y\geq X + \tau] + 0.5 \Prob\left[|Y- X| < \tau\right]}{\Prob[X\geq Y + \tau] + 0.5 \Prob\left[|Y- X| < \tau\right]}\).
\end{itemize}
...
Testing a net treatment benefit of 0, a win odds of 1, or a
 ...
 approximations. Such discrepancies will not arise when using
...
1  squamous   300 169.40260    103.6104
2 smallcell   540 150.00000    246.7778
3     adeno   162  56.00000     72.7500
4     large   180  50.83333    117.8333
\end{verbatim}
...
\Warning The approach is also applicatble to the probabilistic index
but not to the win ratio/odds: the ratio between the global
proportions is taken, i.e., pooling is performed at the numerator and
at the denominator instead of pooling fractions - see
\cite{dong2018stratified}, equation 1.
...
conditional interpretation, as it summarizes comparisons made between
...
\subsection{Using multiple endpoints (hierarchical)}
\label{sec:orgb759520}
...
\Warning As mentionned in chapter 5, section 5.5.2 of
\cite{buyse2025gpc}, in presence of right-censoring and when using a
probabilistic scoring rule (e.g. Peron's scoring rule), the joint
probability across endpoints is approximated by the product of the
endpoint-specific marginal probabilities, e.g.:
\begin{align*}
\widehat{\Prob}[Y_2> X_2 + \tau, |X_1-Y_1|<\tau_1] &= \widehat{\Prob}[Y_2> X_2 + \tau| |X_1-Y_1|<\tau_1]\widehat{\Prob}[|X_1-Y_1|<\tau_1] \\
                                                  &\approx \widehat{\Prob}[Y_2> X_2 + \tau] \widehat{\Prob}[|X_1-Y_1|<\tau_1]
\end{align*}

When working with multiple, correlated, censored endpoints this
approximation can lead to some bias. See the remainder of chapter 5 of
\cite{buyse2025gpc} for a more principled estimation method that is
however not yet implemented in BuyseTest. A point estimate could
however be obtained from \texttt{BuyseTest} by providing an appropriate
survival model in the \texttt{model.tte} argument.

\subsection{Using multiple endpoints (non-hierarchical)}
\label{sec:org8088727}
...
2    karno     1e-12    0.2   100  41.81586    44.94885 13.23529     0 -0.03132992
        Delta   lower.ci   upper.ci   p.value
1 -0.07012668 -0.2203714 0.08336855 0.3707289
2 -0.07639267 -0.2503756 0.10237001 0.4026905
\end{verbatim}
...
Alternatively, one may be interested in the endpoint
specific results. This can be performed by applying the \texttt{BuyseTest}
function separately to each endpoint, e.g.:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
Note: the apparent discrepancy in p-value between the hierarchical and
...
\subsection{What about a multiplicative instead of additive threshold?}
\label{sec:orgd418ae7}

If the interest lies in \(\Prob[Y \geq \tau X ]-\Prob[X \geq \tau Y
]\) instead of \(\Prob[Y \geq X + \tau]-\Prob[X \geq Y + \tau]\) one
should specify a \texttt{*} in the operator:
...
\begin{verbatim}
endpoint threshold total(%) favorable(%) unfavorable(%) neutral(%) uninf(%)   Delta
    time       *20      100          5.3           6.13      88.57        0 -0.0083
CI [2.5% ; 97.5%] p.value
  [-0.068;0.0515] 0.78637
\end{verbatim}


To have a multiplicative threshold when smaller values are favorable use \texttt{*<0}.

\clearpage

\subsection{Stopping comparison for neutral pairs}
\label{sec:orgaa9eaa5}
...
\subsection{Extracting the contribution of each pair to the statistic (long format)}
\label{sec:orga95f057}
...
of each pair in a data.frame format (one row = one pair). For instance
the following code extracts the contribution for the first endpoint:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\subsection{Extracting the contribution of each pair to the statistic (matrix format)}
\label{sec:org2ef9f13}
It is also possible to extract the results of the comparisons in a matrix format:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
where the rows correspond to the 'placebo' observations and the colums
to the 'experimental' observations. To obtain all possible pairwise
comparisons, including within group, one can set the argument \texttt{within}
to \texttt{TRUE}:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
We then obtain a squared matrix with as many row and columns as
observations (Placebo and Experimental) that can be divided in 3 blocks:

\medskip

\begin{minipage}{0.7\linewidth}
\begin{itemize}
\item A: within 'placebo' comparisons (0 on the diagonal)
\item B: across group comparisons
\item C: within 'experimental'' comparisons (0 on the diagonal), \newline identical to
when the argument \texttt{within} was \texttt{FALSE} (default value)
\end{itemize}
\end{minipage}
...
\begin{verbatim}
      [,1] [,2]    [,3] [,4] [,5]    [,6]    [,7]    [,8]    [,9]
 [1,]    0    1  1.0000  1.0   NA -1.0000 -1.0000 -1.0000 -1.0000
 [2,]   -1    0 -1.0000 -1.0   NA -1.0000  1.0000  1.0000  0.0000
 [3,]   -1    1  0.0000 -1.0   NA -0.9600  0.6304  0.0802 -0.4700
 [4,]   -1    1  1.0000  0.0   NA -1.0000  0.2000 -0.9000 -1.0000
 [5,]   NA   NA      NA   NA   NA      NA      NA      NA      NA
 [6,]    1    1  0.9600  1.0   NA  0.0000 -1.0000 -0.9076 -0.8000
 [7,]    1   -1 -0.6304 -0.2   NA  1.0000  0.0000  0.6740  1.0000
 [8,]    1   -1 -0.0802  0.9   NA  0.9076 -0.6740  0.0000  0.5338
 [9,]    1    0  0.4700  1.0   NA  0.8000 -1.0000 -0.5338  0.0000
\end{verbatim}

\bigskip

More examples and explanation can be found in the documentation of the
method \texttt{getPairScore} and \texttt{getMatrixScore}.

\subsection{Extracting the survival probabilities}
\label{sec:org4217088}
...
In presence of censoring or missing values, it is often not be
 ...
 of 0, unless re-analyzed at a lower priority endpoint. Consider the following
 dataset:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
set.seed(10)
...
\noindent We would like to be able to recover the net treatment benefit estimated with the uncensored event times:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
eventtimeUncensored_t0.5 
                  -0.271
\end{verbatim}


using the censored survival times. The \texttt{scoring.rule} argument
indicates how pairs involving missing data are compared:
\begin{itemize}
...
observed values. This require a model for the survival
distribution/cumulative distribution function which, by default,
will be a Kaplan Meier estimator stratified on treatment and GPC
\texttt{strata} variable (if any).
...
tail is unknown. Only possible when using a (stratified)
Kaplan-Meier estimator and no competing risks.
\item \textbf{the Latta's scoring rule} same as the Peron's scoring rule
except that the survival curve/ cumulative distribution function
is assumed to be the same across treatment groups. Only possible
when using a Kaplan-Meier estimator.
\item \textbf{Parametric scoring rule} same as the Peron's scoring rule
except that the survival curve is assumed to follow a parametric
distribution (e.g. Exponential or Weibull). Only available when
there are no competing risks. Based on \texttt{survival::survreg} which
is compatible with the following distributions:
\end{itemize}
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
[1] "extreme"     "logistic"    "gaussian"    "weibull"     "exponential" "rayleigh"   
[7] "loggaussian" "lognormal"   "loglogistic" "t"
\end{verbatim}


Uninformative score will typicall arise with the Gehan's scoring rule
  and, in presence of administrative censoring, with Peron's/Latta's
  scoring rule as the survival curve is not entirely
  identified. Without lower-priority endpoint this will typically bias
  the estimates (e.g. of the Net Treatment Benefit) towards 0
  \footnote{While the power is typically reduced, the type 1 error will
  typically still be controled under independent between the censoring
  and event of interest process, conditionally on strata variables (if
  any)}. As a remedie one can:
\begin{itemize}
\item use a \textbf{restriction time} within the time interval where the survival
curve can be estimated for each group.
\item use a lower priority endpoint.
\item use parametric assumption in survival model to defined the survival
curve over all timepoints. This can be achieved via the \texttt{model.tte}
argument or using the Efron's/Weibull's/\ldots{} scoring rule.
\item use a proportional hazard assumption to \textbf{impute the uninformative
scores}. This can be achieved via the \texttt{correction.uninf} argument
\texttt{1}.
\end{itemize}

The first two solutions lead to a change of estimand, the first being
clearly defined while the definition of the second depends of the
censoring distribution. The last two solutions correspond make instead
statistical assumptions, the former assumptions being more explicit
than with the later solution. The following subsection exemplify some
of the previous possibilities on an illustrative dataset.

\subsection{Gehan's scoring rule}
\label{sec:org96d0e8d}
...
\subsection{Latta's scoring rule}
\label{sec:orgbc1a6ad}

The Latta's scoring rule:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
   endpoint threshold total favorable unfavorable  neutral uninf      Delta  lower.ci
1 eventtime       0.5   100  17.52701    36.13826 46.33473     0 -0.1861125 -0.286479
     upper.ci      p.value
1 -0.08170553 0.0005240922
\end{verbatim}


leads to an estimate closer to the truth, but still attenuated
towards 0. This is not surprinsing as under the alternative we
'impute' the censored pairs under the null. In this specific example
there are no uninformative pairs as the observation with the largest
event time was an event:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
   id treatment eventtimeUncensored eventtime status toxicity eta_toxicity status1
1: 72         C            2.668629  2.668629      1      yes    -1.925644       1
\end{verbatim}


In other cases, and in presence of administrative censoring, they will
  be uniformative pairs.

\noindent \uline{Note:} this scoring rule can be 'manually' implemented by
first fitting a non-parametric survival model common to both treatment
groups via \texttt{prodlim}:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
Then passing the survival model to \texttt{BuyseTest} via the \texttt{model.tte} argument:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
                 estimate         se  lower.ci    upper.ci null      p.value
eventtime_t0.5 -0.1861125 0.05241572 -0.286479 -0.08170553    0 0.0005240922
\end{verbatim}

\subsection{Peron's scoring rule}
\label{sec:org94a634f}
...
leads to an estimate quite close to the truth, despite still having
some uninformative pairs (about 1.37\%). Indeed the last observation in
one group is a drop-out:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
so the full survival curve could not be identified.

\bigskip

\noindent \uline{Note:} this scoring rule can be 'manually' implemented by
first fitting a non-parametric survival model specific to each
treatment group via \texttt{prodlim}:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
library(prodlim)
...
\begin{verbatim}
                 estimate         se   lower.ci   upper.ci null      p.value
eventtime_t0.5 -0.3216337 0.07403487 -0.4584262 -0.1699543    0 5.385074e-05
\end{verbatim}
...
Consider adding an attribute "iidNuisance" to the argument 'model.tte' taking value TRUE to change this default behavior.
                 estimate         se   lower.ci   upper.ci null      p.value
eventtime_t0.5 -0.3216337 0.05153296 -0.4187087 -0.2172912    0 6.570106e-09
\end{verbatim}
...
\subsection{Efron's scoring rule}
\label{sec:org9d66204}

The Efron's scoring rule:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
   endpoint threshold total favorable unfavorable  neutral uninf      Delta  lower.ci
1 eventtime       0.5   100   11.1737    43.97856 44.84774     0 -0.3280486 -0.466052
    upper.ci      p.value
1 -0.1744621 4.871577e-05
\end{verbatim}


leads to no uniformative pairs as the survival is set to 0 after the
last observation. 

\uline{Note:} an add-hoc modification of the data can be used to 'manually'
implement this scoring rule. Recalling that the tail of the survival
curve is unknown only in the treatment group:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
dt[,.SD[which.max(eventtime)],by="treatment"]
\end{lstlisting}
...
We can set the last censored observation(s) in that group to have the
event shortly after the censoring time:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
dtEfron <- copy(dt)
...
Estimate the corresponding non-parametric survival model via \texttt{prodlim}:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
and run the GPC with this survival model:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.Efron <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
...
\begin{verbatim}
                 estimate         se  lower.ci   upper.ci null      p.value
eventtime_t0.5 -0.3280486 0.07484093 -0.466052 -0.1744621    0 4.871577e-05
\end{verbatim}

\subsection{Scoring rule based on a parametric survival model}
\label{sec:org6ff190f}

Parametric survival distribution specified in the \texttt{survreg} function of the survival package:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
can be used to model the group-specific survival and plugged in the
...
\begin{verbatim}
   endpoint threshold total favorable unfavorable  neutral       uninf      Delta   lower.ci
1 eventtime       0.5   100  13.54196    41.34855 45.10801 0.001482627 -0.2780659 -0.4192085
    upper.ci      p.value
1 -0.1237996 0.0005138051
\end{verbatim}


which is very close to the true value. This is not surprising since we
used a parametric survival model compatible with the data generating
mechanism (exponential distribution). In theory there should be no
uninformative pairs. However \texttt{BuyseTest} discretise the (continuous)
survival distribution into a piecewise constant distribution and the
uniformative pairs corresponds to the density beyond the last
discretisation point. Internally the survival curve is discretized
using 1000 points starting from survival = 1 to survival = 0.001. A
greater resolution can be specified with a number following the
distribution name:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.Weibull <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
          data = dt, scoring.rule = "Weibull 10000", trace = 0)
model.tables(e.Weibull)
\end{lstlisting}

\begin{verbatim}
   endpoint threshold total favorable unfavorable  neutral        uninf      Delta   lower.ci
1 eventtime       0.5   100  13.52321    41.34697 45.12977 4.310969e-05 -0.2782376 -0.4193279
    upper.ci      p.value
1 -0.1240234 0.0005078037
\end{verbatim}


\noindent \uline{Note:} this scoring rule can be 'manually' implemented by
first fitting a parametric survival model specific to each treatment
group via \texttt{survreg}, e.g.:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
library(survival)
...
The method \texttt{BuyseTTEM} can be used to obtain a discrete time version
of the parametric survival model:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.TTEMWeibull <- BuyseTTEM(e.survregWeibull, treatment = "treatment",
                           n.grid = 10000, iid = TRUE)

\end{lstlisting}

Here the argument \texttt{n.grid} controls how many points are used to
discretise the survival curve.


\clearpage

Then we can pass the model to the \texttt{BuyseTest} via the \texttt{model.tte}
argument:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.WeibullManual <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
                             model.tte = e.TTEMWeibull,
                             data = dt, scoring.rule = "Peron", trace = 0)
...
\subsection{Correction via re-weighting \citep{peron2021correcting}}
\label{sec:orgbff698b}

\subsubsection{At the trial level}
\label{sec:org3e8a3bc}

The score of informative pairs is inflated proportionnally to the
amount non-informative score, e.g. as in
\cite{buyse2008reformulating}. It can be seen as right-censoring under a
proportional hazard-like assumption. While this can be reasonnable if
there are no lower priority endpoints, it will disregard lower
priority information of pairs classified uniformative at the higher
priority endpoint. \newline \Warning The current implementation does
not account for the uncertainty about the inflation factor when using
\texttt{method.inference} equal to \texttt{"U-statistic"}.

\bigskip

In the present example, the correction leads to an estimate:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
reasonnably close to the true net treatment benefit. The correction
could also be applied with Peron's scoring rule. However since there
were 'only' 1.37\% uniformative pair it will not substantially affect
the results of this analysis. To get some inside about the correction
we can look at the scores of the pairs:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
iScore <- getPairScore(BT.trialW, endpoint = 1)
iScore[,.SD[1], .SDcols = c("favorableC","unfavorableC","neutralC","uninfC"),
...
\subsubsection{At the pair level}
\label{sec:org6cdf883}

This score of the uniformative pairs is set the average favorable,
unfavorable, and neutral probability observed on the sample:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
In the present example it leads to exactly the same result since there
is only one endpoint. But in presence of multiple endpoints the
neutral score of the uninformative pairs would be analysed based on
the observed lower-priority endpoint of these uninformative pairs,
which makes it better suited to analyze multiple endpoints. We can see
the neutral score 'imputed' to each uniformative pair (line 3):
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
which matches the proportion of favorable, unfavorable, and neutral pairs of the GPC analysis:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
0.1182278   0.3643038   0.5174684
\end{verbatim}

\subsubsection{Note on the use of the corrections}
\label{sec:org6c3b862}

As mentioned in \cite{peron2021correcting}, the corrections (at the
trial or pair level) assumes that uninformative pairs would on average
...
We first evaluate the 'oracle' net treatment benefit, i.e. the one we
would have estimated with complete data:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
Error in .local(object, ...) (from S4-BuyseTest-summary.R#155) : object 'o' not found
1: summary(BT)
2: summary(BT)
3: .local(object, ...)
\end{verbatim}
