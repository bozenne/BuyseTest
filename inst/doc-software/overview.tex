...
[1] prodlim_2026.03.11  ggplot2_4.0.2       survival_3.5-8      BuyseTest_3.3.8    
...
[37] colorspace_2.1-2          future_1.70.0             riskRegression_2026.04.22
...
[67] testthat_3.3.2            foreach_1.5.2             cmprsk_2.2-12            
[70] snow_0.4-4                glue_1.8.1                parallelly_1.47.0        
[73] codetools_0.2-19          stringi_1.8.7             shape_1.4.6.1            
[76] gtable_0.3.6              tibble_3.3.1              pillar_1.11.1            
[79] brio_1.1.5                quantreg_6.1              htmltools_0.5.9          
[82] lava_1.9.0                R6_2.6.1                  rprojroot_2.1.1          
[85] evaluate_1.0.5            lattice_0.22-5            backports_1.5.1          
[88] memoise_2.0.1             MatrixModels_0.5-4        nlme_3.1-163             
[91] gridExtra_2.3             checkmate_2.3.4           xfun_0.57                
[94] zoo_1.8-15                fs_2.1.0                  pkgconfig_2.0.3
\end{verbatim}

\clearpage

\section{Performing generalized pairwise comparisons (GPC)}
\label{sec:org546a79a}
...
\Warning Testing an inadequate
null hypothesis can have dramatic consequences on the p-value:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{verbatim}
 ...
 - inference       : H-projection of order 1 after atanh transformation 
 - treatment groups: Exp (treatment) vs. Pl (control) 
 - strata weights  : 18.47%, 20.52%, 26.38%, 34.63% 
...
\begin{verbatim}
 ...
 - inference       : H-projection of order 1 after atanh transformation 
 - treatment groups: Exp (treatment) vs. Pl (control) 
 - strata weights  : 13.71%, 15.23%, 25.38%, 45.69% 
 - uninformative pairs: no contribution
 - results
 endpoint    strata total(%) favorable(%) unfavorable(%) neutral(%) uninf(%)   delta   Delta
...
[1] 137 137
     [,1] [,2]    [,3] [,4]    [,5]    [,6]    [,7]    [,8]
[1,]    0    1  1.0000  1.0 -1.0000 -1.0000 -1.0000 -1.0000
[2,]   -1    0 -1.0000 -1.0 -1.0000  1.0000  1.0000  0.0000
[3,]   -1    1  0.0000 -1.0 -0.9600  0.6304  0.0802 -0.4700
[4,]   -1    1  1.0000  0.0 -1.0000  0.2000 -0.9000 -1.0000
[5,]    1    1  0.9600  1.0  0.0000 -1.0000 -0.9076 -0.8000
[6,]    1   -1 -0.6304 -0.2  1.0000  0.0000  0.6740  1.0000
[7,]    1   -1 -0.0802  0.9  0.9076 -0.6740  0.0000  0.5338
[8,]    1    0  0.4700  1.0  0.8000 -1.0000 -0.5338  0.0000
\end{verbatim}
...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
rbind(cbind(MscoreAll[1:4,1:4],MscoreAll[70:73,1:4]),
      cbind(MscoreAll[1:4,70:73],MscoreAll[70:73,70:73]))
\end{lstlisting}
\bigskip
...
\subsection{Extracting the score matrix}
\label{sec:org3af776d}

\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
veteran.ext <- rbind(cbind(XXgroupXX = "A", XXindexXX = 1:NROW(veteran), veteran), 
                     cbind(XXgroupXX = "B", XXindexXX = 1:NROW(veteran), veteran))

GPC.ext <- BuyseTest(XXgroupXX ~ tte(time,status, threshold = 20), data = veteran.ext, 
                     method.inference = "none", keep.pairScore = TRUE, trace = FALSE)
endpoint.ext <- GPC.ext@endpoint
dfScore.ext <- getPairScore(GPC.ext, endpoint = 1:length(endpoint.ext), cumulative = TRUE, unlist = FALSE)        
...
distribution which, by default, will be a Kaplan Meier estimator
stratified on treatment and GPC \texttt{strata} variable (if any).
...
except that the survival curve is assumed to be the same across
treatment groups. Only possible when using a Kaplan-Meier
estimator and no competing risks.
...
distribution (e.g. Exponential or Weibull).
\end{itemize}

Uninformative score will typicall arise with the Gehan's scoring rule
  and, in presence of administrative censoring, with
  Peron's/Efron's/Latta's scoring rule as the survival curve is not
  entirely identified. Without lower-priority endpoint this will
  typically bias the estimates (e.g. of the Net Treatment Benefit)
  towards 0 \footnote{While the power is typically reduced, the type 1
  error will typically still be controled under independent between
  the censoring and event of interest process, conditionally on strata
  variables (if any)}. As a remedie one can:
\begin{itemize}
...
of the previous possibility on the illustrative dataset.

\subsection{Gehan's scoring rule}
\label{sec:org9f3cf3f}
...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.fineregWeibull <- BuyseTTEM(e.survregWeibull, treatment = "treatment", iid = TRUE, n.grid = 2500)
str(e.fineregWeibull$peron$jumpSurvHaz[[1]][[1]])
\end{lstlisting}
...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.WeibullFine <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
                           model.tte = e.fineregWeibull,
                           data = dt, scoring.rule = "Peron", trace = 0)
model.tables(e.WeibullFine)
\end{lstlisting}
...
It is therefore possible to extend the approach to other models by
