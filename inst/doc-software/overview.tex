...
\label{sec:orgd7ffe4d}

The Latta's scoring rule:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.L <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
...
\noindent \uline{Note:} this scoring rule can 'manually' implemented by a survival
model common to both groups:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.prodlim1 <- prodlim(Hist(eventtime, status) ~ 1, data = dt)
attr(e.prodlim1,"iidNuisance") <- TRUE
\end{lstlisting}

and run the 'Peron scoring rule' GPC with this survival model:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.L1 <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
                  model.tte = e.prodlim1,
...
\noindent \uline{Note:} the censoring model can be specified by first fitting a
...
\label{sec:orga59eb01}

The Efron's scoring rule:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.E <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
...
\uline{Note:} an add-hoc modification of the data can be used to perform
this scoring rule. Recalling that the tail of the survival curve is
unknown only in the treatment group:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
dt[,.SD[which.max(eventtime)],by="treatment"]
\end{lstlisting}
...
\begin{lstlisting}
dt2 <- copy(dt)
dt2[treatment=="T" & id == 154, c("eventtime","status") := .(eventtime+1e-10,1)]
## dt2[treatment=="T" & eventtime > 1.58865, status := 1]
\end{lstlisting}

Estimate the corresponding survival model:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
e.E1 <- BuyseTest(treatment ~ tte(eventtime, status, threshold = 0.5),
