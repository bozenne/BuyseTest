...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
strata.obs <- nobs(BTstrata, stratified = TRUE)
strata.obs
\end{lstlisting}
...
one can specify the argument \texttt{stratified} to extract strata-specific estimates:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
the threshold of clinical relevance. The \texttt{confint} method will use a
...
tested by \texttt{confint} is generally not meaningful and the p-value should
...
should set the argument \texttt{add.halfNeutral} to \texttt{TRUE} when calling
\texttt{BuyseTest}:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
