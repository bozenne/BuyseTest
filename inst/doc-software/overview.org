...
\tau]\) leads to the net treatment benefit[fn::in absence of ties and
...
- the function =BuyseTest= performs the GPC procedure and is the main
  ...
  various methods:
    ...
    + =confint= to extract estimates, confidence intervals, and
      p.values.
    + =plot= for a graphical display of the scoring of the pair per
      endpoint.
    + =sensitivity= to perform a sensitivity analysis on the choice of
      the threshold(s).
    + =nobs= to extract the number of observations and pairs.
    + =getIid= to extract the iid decomposition of the estimator.
    + =getPairScore= to extract the contribution of each pair in a data.frame format (long format).
    + =getMatrixScore= to extractthe contribution of each pair in a matrix format (squared format).
...
Before going further we need to load the *BuyseTest* package in the \Rlogo
session:
...
To illustrate the functionalities of the package, we use the =veteran=
...
The =model.tables= method outputs the information from the previous
table in a =data.frame= format:
...
- Wilcoxon-Mann-Whitney parameter or probabilistic index: \(\Prob[Y\geq X + \tau] + 0.5 \; \Prob\left[|Y- X| < \tau\right]\).
- win odds: \(\frac{\Prob[Y\geq X + \tau] + 0.5 \Prob\left[|Y- X| < \tau\right]}{\Prob[X\geq Y + \tau] + 0.5 \Prob\left[|Y- X| < \tau\right]}\).
...
however be obtained from =BuyseTest= by providing an appropriate
survival model in the =model.tte= argument.
...
specific results. This can be performed by applying the =BuyseTest=
...
should specify a =*= in the operator:
...
To have a multiplicative threshold when smaller values are favorable use =*<0=.

#+BEGIN_SRC R :exports none :results output :session *R* :cache no
BuyseTest(trt ~ tte(time, status = "status", threshold = 20, operator = "*<0"), data = veteran)
#+END_SRC
...
comparisons, including within group, one can set the argument =within=
to =TRUE=:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
- A: within 'placebo' comparisons (0 on the diagonal)
- B: across group comparisons 
- C: within 'experimental'' comparisons (0 on the diagonal), \newline identical to
  when the argument =within= was =FALSE= (default value)
#+LaTeX: \end{minipage}
#+LaTeX: \begin{minipage}{0.27\linewidth} 
#+BEGIN_EXPORT latex
\begin{align*}
\begin{bmatrix}
A & B \\ \trans{B} & C
...
method =getPairScore= and =getMatrixScore=.
...
using the censored survival times. The =scoring.rule= argument
indicates how pairs involving missing data are compared:
    ...
    + *the Latta's scoring rule* same as the Peron's scoring rule
    ...
    + *Parametric scoring rule* same as the Peron's scoring rule
      ...
      there are no competing risks. Based on =survival::survreg= which
...
Uninformative score will typicall arise with the Gehan's scoring rule
  ...
  [fn::While the power is typically reduced, the type 1 error will
...
- use a lower priority endpoint.
- use parametric assumption in survival model to defined the survival
  curve over all timepoints. This can be achieved via the =model.tte=
  argument or using the Efron's/Weibull's/... scoring rule.
- use a proportional hazard assumption to *impute the uninformative
  scores*. This can be achieved via the =correction.uninf= argument
  =1=.
...
\noindent _Note:_ this scoring rule can be 'manually' implemented by
...
Then passing the survival model to =BuyseTest= via the =model.tte= argument:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
_Note:_ an add-hoc modification of the data can be used to 'manually'
...
Estimate the corresponding non-parametric survival model via =prodlim=:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
Parametric survival distribution specified in the =survreg= function of the survival package:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
uninformative pairs. However =BuyseTest= discretise the (continuous)
...
The method =BuyseTTEM= can be used to obtain a discrete time version
...
Here the argument =n.grid= controls how many points are used to
discretise the survival curve.


\clearpage

Then we can pass the model to the =BuyseTest= via the =model.tte=
argument:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
...
=method.inference= equal to ="U-statistic"=.
