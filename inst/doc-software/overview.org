...
_Note:_ while it is possible to extract the strata-specific estimate
(e.g. =coef(BTstd, strata = TRUE)=) the software does not keep track
of the strata-specific uncertainty via the H-decomposition. It will
output an error message when requesting it (e.g. =confint(BTstd,
strata = TRUE)=). A resampling method should be used instead.

#+BEGIN_SRC R :exports none :results output :session *R* :cache no
XXXX <- BuyseTest(ffstrata, data = veteran, pool.strata = "standardization", method.inference = "bootstrap",
                  n.resampling = 100)
confint(XXXX, strata = TRUE)
#+END_SRC
