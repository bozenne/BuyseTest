...
\label{sec:orgf43a13b}

\subsection{Univariate case}
\label{sec:orgb72dcf5}
...
outcome in each group \(x_k \succ y_k = \Ind[x_k \geq y_k + \tau_k]\) where \(\Ind[.]\) denotes the indicator function. We can then
denote by:
\begin{align*}
...
\subsubsection{In R}
\label{sec:orga6dadd9}
\begin{enumerate}
\item No censoring
\label{sec:org03f30bc}
Setting:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
alphaE.X <- 2
...
\subsection{Variance estimator based on the first order H-decomposition}
\label{sec:org42e8b62}
...
\subsection{Variance estimator based on the second order H-decomposition}
\label{sec:org53e4128}
...
\label{sec:org2ea8f1c}

\subsubsection{Binary endpoint}
\label{sec:orge15c7fc}
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
tpsBin <- system.time(
