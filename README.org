...
second endpoint:
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
BT.pair <- BuyseTest(treatment ~ bin(tumor) + cont(size), data = dt.sim,
...
#+RESULTS:
#+begin_example
Error in validNumeric(dataT[[endpoint[iterY]]], name1 = endpoint[iterY],  : 
  BuyseTest: 'tumor' must be numeric 
is(tumor) : character vector data.frameRowLabels SuperClassMethod index atomicVector
 ...
 endpoint threshold total favorable unfavorable neutral uninf delta Delta
    tumor       0.5   100         0         100       0     0    -1    -1
     size     1e-12     0         0           0       0     0     0    -1
#+end_example
...
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
BT.keep <- BuyseTest(trt ~ tte(time, threshold = 20, censoring = "status"),
                     data = veteran, keepComparison = TRUE, 
                     trace = 0, n.resampl)
#+END_SRC
